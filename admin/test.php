...
try {
    ...
    **************************************/
    ...
    foreach ($tableFields as $key => $type) {
        $createTableFields[] = "$key $type";
    }
    $createTableFields = implode(",", $createTableFields);

    $file_db->exec("CREATE TABLE IF NOT EXISTS {$tableName} (
                    id INTEGER PRIMARY KEY, {$createTableFields} )");


  // check if email in table
  ...
  */
  $findEmail = $email;
  $stmt = $file_db->prepare("SELECT COUNT(*) FROM {$tableName} WHERE email = :email");
  // $stmt->bindParam(":email", $email);
